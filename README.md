...
重载页面时拦截前端脚本响应，仅在内存中启用官方已经提供的中文资源。
...
- 不修改 WindowsApps 中的官方文件；退出应用后，内存补丁随进程消失。
...
官方应用升级可能改变国际化开关。启动器只有在实际匹配并修改语言门控、且确认中文
环境脚本生效后才报告成功；未知版本匹配失败时会停止并显示诊断信息。用户应关闭通过
本启动器打开的 ChatGPT，并改用官方快捷方式。
